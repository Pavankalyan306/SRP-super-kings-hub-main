...
import { useBallsRealtime, RealtimeBall } from '@/hooks/useRealtime';
import { useAuth } from '@/context/AuthContext';
import { motion, AnimatePresence } from 'framer-motion';
import { AlertCircle, CheckCircle2, Loader2, Plus, Zap, Signal, Wifi } from 'lucide-react';
import { BallData } from '@/lib/balls';

interface EnhancedBallRecorderProps {
  matchId: string;
  onSuccess?: () => void;
}

export default function EnhancedBallRecorder({ matchId, onSuccess }: EnhancedBallRecorderProps) {
  const { user } = useAuth();
  ...
  // Realtime subscription with callbacks
  const handleRealtimeBallInsert = useCallback((newBall: RealtimeBall) => {
    console.log('🔴 Live ball update received:', newBall);
  }, []);

  const { isConnected, error: realtimeError } = useBallsRealtime(matchId, {
    onInsert: handleRealtimeBallInsert,
    onUpdate: (ball) => console.log('Ball updated:', ball),
    onDelete: (ball) => console.log('Ball deleted:', ball),
  });
  ...
  const [recentBalls, setRecentBalls] = useState<RealtimeBall[]>([]);
      ...
      >
        {isConnected ? (
          <>
            ...
            <div>
              <p className="text-sm font-semibold text-green-700">Live Connection Active</p>
              <p className="text-xs text-green-600">Receiving real-time ball updates</p>
            </div>
          </>
        ) : (
          <>
            ...
            <div>
              <p className="text-sm font-semibold text-yellow-700">Connecting...</p>
              <p className="text-xs text-yellow-600">
                {realtimeError || 'Establishing live connection'}
              </p>
            </div>
          </>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            {/* Over Number */}
            <div>
              <label className="block text-xs font-medium text-foreground mb-1">Over *</label>
              <input
                type="number"
                name="over_number"
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  min="0"
                  max="6"
                  className={`flex-1 px-3 py-2 rounded-lg bg-secondary text-foreground text-sm border ${
                ...
                <div className="flex gap-1">
                  {[0, 1, 2, 4, 6].map((run) => (
                    <button
                      key={run}
                      type="button"
                      onClick={() => setFormData((prev) => ({ ...prev, runs: run }))}
                      disabled={isPending}
        ...
        {/* Balls List with Realtime Updates */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-foreground">Live Ball Updates</h2>
            {isConnected && (
              <motion.div
              ...
              <AnimatePresence>
                {balls.map((ball, idx) => (
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: -20 }}
                    className="p-3 bg-secondary rounded-lg border border-border/50 hover:border-primary/50 transition-colors"
                  >
          ...
          {balls && balls.length > 0 && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="mt-4 pt-4 border-t border-border grid grid-cols-3 gap-2"
            >
              <div className="text-center p-2 bg-primary/10 rounded">
                <p className="text-xs text-muted-foreground">Total Runs</p>
                <p className="text-lg font-bold text-primary">
                  {balls.reduce((sum, ball) => sum + (ball.runs || 0), 0)}
                </p>
              </div>
              <div className="text-center p-2 bg-primary/10 rounded">
                <p className="text-xs text-muted-foreground">Balls</p>
                <p className="text-lg font-bold text-primary">{balls.length}</p>
              </div>
              <div className="text-center p-2 bg-destructive/10 rounded">
                <p className="text-xs text-muted-foreground">Wickets</p>
                <p className="text-lg font-bold text-destructive">
                  {balls.filter((b) => b.wicket).length}
                </p>
              </div>
            </motion.div>
          )}
